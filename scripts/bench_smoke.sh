...
timeout --preserve-status "$((DURATION + 5))s" "${BIN}" --iface "${HOST_IF}" --mode encrypt --key "${KEY}" --port "${UDP_PORT}" --stats-interval 1 --duration "$((DURATION + 1))" >"${LOG}" 2>&1 &
APP_PID=$!
sleep 2
...
ip netns exec "${NS}" python3 - "${NS_IF}" "${NS_MAC}" "${HOST_MAC}" "${UDP_PORT}" "${DURATION}" "${COUNT_FILE}" <<'PY'
import socket
import struct
import sys
import time

iface, src_mac_text, dst_mac_text, dst_port_text, duration_text, count_path = sys.argv[1:]
